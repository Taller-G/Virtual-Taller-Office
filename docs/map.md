...
- The server's tests require at least those four zones, plus the three rooms in the south wing, and
  ...
  not repeated) plus that the `Lobby` exists.
...
| `chiron-office` | `chiron-office.json` | Lobby, Monitors, Archive, Gallery, The Pit, War Room and Night Café             |

They are connected by **a pair of doors**, one at each end:

| From            | Door (tile) | Leads to        | Arrival point (tile)       |
| --------------- | ----------- | --------------- | -------------------------- |
| `first-office`  | (25, 3)     | `chiron-office` | `from-first-office` (5, 4) |
| `chiron-office` | (5, 2)      | `first-office`  | `from-chiron` (25, 4)      |

Both sit in their world's reception / lobby, against the north wall, and both have an **opening**
...
The second world: dark, cold and open-plan. The floor plan deliberately looks nothing like the First
Office's — over there closed rooms hang off a vertical corridor on a square 40×40 canvas; here the
canvas is landscape (34×24) and everything gives onto an east-west **gallery**, with alcoves
separated by short stub walls and pillars, and no interior doors.
