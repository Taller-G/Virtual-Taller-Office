...
What: Over each door rectangle there is a decorative two-tile `portal` sprite (from `ChironDark`) on the wall above it, plus a `threshold` glow on the floor tile; the First Office map embeds `ChironDark` just for those three tiles · Why: the client already draws a violet rectangle and the destination world's name over a door, but on its own that reads as a marker, not as a way out; a dark opening with cold light spilling from it is legible at a glance and says _which_ world is on the other side · Where: apps/client/public/assets/map/oficina-taller.json (layer Muebles), tools/make-chiron-map.py (`portal`) · Learned: only the north wall of a room has a visible face in this art style — south and side walls are a one-tile line seen edge-on, so a doorway cut into them does not read; put the door in the north wall
